...
class ReinforceAgent(BaseAgent):
    ...
    def initialize_policy(self):
        pass
    def update_policy(self, state, action, reward, new_state, terminated, policy_response_dict:dict):
        terminated_last_step = self.replay_buffer.get_latest_termination_state_and_check_for_max_episode_length()
        if terminated_last_step:
            episode_history = self.replay_buffer.get_episode_hist()
            rel_episode_hist = episode_history.clone()
            rel_rewards = rel_episode_hist[self.actor_network.input_dim + self.n_actions, :].clone()
            log_prob_idx = (2 * self.actor_network.input_dim + self.n_actions + 2)
            # ('state','action','reward',next_state','terminated', log_probs, entropy)
            rel_log_probs = rel_episode_hist[log_prob_idx: log_prob_idx + self.n_actions, :].clone()
            ...
            if self.parser.args.grad_clipping:
                torch.nn.utils.clip_grad_norm_(
                    [p for g in self.actor_network.optimizer.param_groups for p in g["params"] ],1,)
