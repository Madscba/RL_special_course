...
def get_agent(argparser, environments):
    action_dim, state_dim, n_actions, action_type = get_environment_info(envs = environments)

    if argparser.args.algorithm == "DQN":
        agent = DQNAgent(argparser=argparser,use_DDQN=False,state_dim=state_dim, action_dim=action_dim,n_actions = n_actions)
        return agent
