...
class RLLogger:
    def __init__(self, argparser, frame_interval:int= 10000, log_format:str ='console',log_frequency:int=100):
        ...
        self.run_name = '__'.join([ key[:4] +'' +str(val)[:5] for key,val in self.run_config.items()])
        self.step_rewards = []
    ...
    def _print_epi_log(self):
        ...
        print(f"Episode {self.episode_counter}: Reward = {reward =:.2f}, Length = {length:.2f}") #, Exp. Rate = {exp_rate:.2f}
    ...
    def _write_to_file(self):
        ...
        with open(f"{os.path.join(os.getcwd(), 'results', 'temporary', f'{self.run_name}.pickle')}", "wb") as outfile:
            pickle.dump(log_data, outfile)
        print(f"Saving log to file: {os.path.join(os.getcwd(), 'results', 'temporary', f'{self.run_name}.pickle')}")
    ...
    def plot_step_rewards(self):
        ...
        rew_intervals = np.array_split(np.array(self.step_rewards), self.run_config['n_steps'] // self.frame_interval)
        ...
        plt.scatter(np.where(avg_rewards == x_max)[0][0], x_max, label="maximum avg. rew: {:.2f}".format(x_max))
