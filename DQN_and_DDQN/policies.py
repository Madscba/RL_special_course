...
class ReplayBuffer:
    def __init__(
        self, capacity: int = 100000, state_dim: int = 4, batch_size: int = 64
    ):
    ...
    def save_event(self, state, action, reward, next_state, terminated):
        self.event_tuples[:, self.event_idx % self.capacity] = torch.from_numpy(
            np.hstack((state, action, reward, next_state, terminated))
        )
        self.event_idx += 1
...
class DQN_agent:
    def __init__(
        ...
        self.replay_buffer = ReplayBuffer(
            batch_size=batch_size, state_dim=self.DQN.input_dim
        )
    ...
    def eps_greedy_policy(self, state):
        if np.random.random() < self.eps:
            ...
            ).item()  # had int cast instead of item()
        return action
    ...
    def update_DQN_replay(self):
        if self.replay_buffer.event_idx >= self.batch_size * 20:
            ...
            states, actions, rewards, new_states, terminated = (
                event_tuples[:n, :],
                event_tuples[n : n + 1, :],
                event_tuples[n + 1 : n + 2, :],
                event_tuples[n + 2 : 2 * n + 2, :],
                event_tuples[2 * n + 2 :, :],
            )
    ...
    def learn_policy(self, n_frames: int = 10000, eval_mode: bool = False):
        ...
        plt.title(
            f'Average episode length, lr:{self.DQN.optimizer.defaults["lr"]*100},n_frames:{n_frames},eps:{eps:.2f},\n eps_decay:{self.eps_decay}, gamma: {gamma} eval_mode: {eval_mode}'
        )
        ...
        plt.title(
            f'Average reward pr. episode, lr:{self.DQN.optimizer.defaults["lr"]},n_frames:{n_frames},eps:{eps:.2f},\n eps_decay:{self.eps_decay:.2f}, gamma: {gamma} eval_mode: {eval_mode}'
        )
        plt.subplots_adjust(bottom=0.2)
        plt.show()
...
if __name__ == "__main__":
    ...
    wrapped_env = RecordVideo(
        ...
        episode_trigger=lambda x: x % 50 == 0 and x > 500,
